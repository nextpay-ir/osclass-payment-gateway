<?php
    ...
    function payment_get_amount($product) {
        ...
        if(count($tmp)>1) {
            if($tmp[0]=='101') {
                return ModelNextPayPayment::newInstance()->getPublishPrice($tmp[1]);
            } else if($tmp[0]=='201') {
                return ModelNextPayPayment::newInstance()->getPremiumPrice($tmp[1]);
            } else  if($tmp[0]=='301') {
                return $tmp[1];
            }
        }
    ...
    function payment_buttons($amount = '0.00', $description = '', $itemnumber = '101', $extra_array = null) {
        if(osc_get_preference('nextpay_enabled', 'payment')==1) {
            NextPay::button($amount, $description, $itemnumber, $extra_array);
        };
    }
