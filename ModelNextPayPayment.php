<?php
     ...
     * @since 3.0
     */
    class ModelNextPayPayment extends DAO
    {
        /**
         * It references to self object: ModelNextPayPayment.
         * It is used as a singleton
         *
         * @access private
         * @since 3.0
         * @var ModelNextPayPayment
         */
        private static $instance ;

        /**
         * It creates a new ModelNextPayPayment object class ir if it has been created
         * before, it return the previous object
         *
         * @access public
         * @since 3.0
         * @return ModelNextPayPayment
         */
        ...
        public function import($file)
        {
            ...
            if(! $this->dao->importSQL($sql) ){
                throw new Exception( "Error importSQL::ModelNextPayPayment<br>".$file ) ;
            }
        }
        ...
        public function install() {
            ...
            osc_set_preference('version', '301', 'payment', 'INTEGER');
            osc_set_preference('default_premium_cost', '20000', 'payment', 'STRING');
            osc_set_preference('allow_premium', '0', 'payment', 'BOOLEAN');
            osc_set_preference('default_publish_cost', '10000', 'payment', 'STRING');
            osc_set_preference('pay_per_post', '0', 'payment', 'BOOLEAN');
            osc_set_preference('premium_days', '30', 'payment', 'INTEGER');
            osc_set_preference('currency', 'ریال', 'payment', 'STRING');
        ...
        public function versionUpdate() {
            ...
            if( $version < 200 ) {
                ...
                $this->dao->query(sprintf('ALTER TABLE %s ADD i_amount BIGINT(20) NULL AFTER f_amount', ModelNextPayPayment::newInstance()->getTable_log()));
                $this->dao->query(sprintf('ALTER TABLE %s ADD i_amount BIGINT(20) NULL AFTER f_amount', ModelNextPayPayment::newInstance()->getTable_wallet()));
                ...
                if($result) {
                    ...
                    foreach($wallets as $w) {
                        $this->dao->update($this->getTable_wallet(), array('i_amount' => $w['f_amount']), array('fk_i_user_id' => $w['fk_i_user_id']));
                    }
                }
                ...
                if($result) {
                    $logs = $result->result();
                    foreach($logs as $log) {
                        $this->dao->update($this->getTable_log(), array('i_amount' => $log['f_amount']), array('pk_i_id' => $log['pk_i_id']));
                    }
                }
